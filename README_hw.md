...
再按 LB 开始 ±0.5 m/s、前进 1.5 秒、后退 1.3 秒的往复；再按 LB 回到摇杆控制。
...
LB 往复命令改为 +0.5/-0.5 m/s，前进 1.5 秒、后退 1.3 秒。RB 的遥控纵向速度档仍为 2.5 m/s，
...
2026-09-10：B 不再逐关节扫 ROM，改为以下两组动作循环：

1. 双臂伸直向左右侧平举，目标是实机与地面水平、左右连成 180°。针对现场反馈抬举不足，肩 x 从左右 ±90° 增至 ±100°（`ARM_LOAD_LATERAL_RAISE_DEG` 可调）；模型中是高于水平 10°，实机水平效果待现场校准，不能把指令当实测角度。肩 z 保持左右 ±90°，手背朝上为目标，然后放下。
2. 上臂向前外侧张开 45°、处于水平面下方 45°；上臂固定，屈肘至大小臂夹角 45°（肘读数 -45°，小于50°），
   再伸肘放下小臂，最后整臂放下。角度以躯干直立时的地面为参考，不做 IMU 姿态补偿。

采用 elf3.xml 的关节约定：肘读数 0° 是大小臂夹角 90°，90° 是伸直。
第二组肩部目标为肩 y=-35.264°、肩 x=左右 ±30°、肩 z=左右 ±35.264°，
不是把三个肩轴都设为 45°。手腕关节保持参考命令；不增加腕部运动。
B 的参考姿态是双臂伸直并各外展 10°（肩 x 左右 ±10°、肘读数 90°），避免紧贴身体，A 仍使用原全零参考；停止 B 会平滑回到 B 参考。

沿用 `suspended_tests.yaml` 的最短移动时间 2.0 秒、端点保持 0.2 秒、峰值速度上限 180°/秒，
两组共 6 段，理论每轮约 13.2 秒，不含首次准备与调度延迟；按 B 开始，再按 B 停止回位。
首次/回位允许误差 10°、运行跟踪告警 2°、反馈超时与命令间隔保护不变。
这不是完整 ROM 验收；5 kg 只是目标负载，程序不会检测负载质量或自动运行满 1 小时。

警告：需要现场核验负载额定能力、法兰紧固、吊具、手臂和腿部净空及急停。
手背朝上按腕局部 +Z 为手背方向设定，XML 没有手掌朝向标记，实际安装朝向仍需空载核对。
